...
def test_grep_logs(ssh_helper):
    ...
    for hots in media_server_pool_4:
        client = ssh_helper.client(host=hots)
        # res = client.exec_command("cat /var/log/ivr/media-server.log | grep 'ERROR\|WARNING'")
        uuid = "beabdb3e-3d49-4e37-8677-cd18c14e4f83"
        res = client.exec_command(
            "cat /var/log/ivr/media-server.log | grep '6f70d16e-8944-4943-86a7-bf3eba050d4a'"
        )
        ...
        #     # res = client.exec_command("cat /var/log/ivr/caller.log | grep 'ERROR\|WARNING'")
        res = client.exec_command(
            "cat /var/log/ivr/caller.log | grep '6f70d16e-8944-4943-86a7-bf3eba050d4a'"
        )
        res_end = str((res[1].read() + res[2].read()), encoding="utf-8")
        print("----CALLER----")
        print(res_end)
    ...
    for hots in logic_pool_4:
        client = ssh_helper.client(host=hots)
        #     # res = client.exec_command("cat /var/log/ivr/logic-executor-offline.log | grep 'ERROR\|WARNING'")
        res = client.exec_command(
            "cat /var/log/ivr/logic-executor-offline.log | grep '6f70d16e-8944-4943-86a7-bf3eba050d4a'"
        )
        res_end = str((res[1].read() + res[2].read()), encoding="utf-8")

        print(hots)

        print("----LOGIC EXECUTOR OFFLINE----")
        print(res_end)

        #     # res = client.exec_command("cat /var/log/ivr/logic-executor-online.log | grep 'ERROR\|WARNING'")
        res = client.exec_command(
            "cat /var/log/ivr/logic-executor-online.log | grep '6f70d16e-8944-4943-86a7-bf3eba050d4a'"
        )
        res_end = str((res[1].read() + res[2].read()), encoding="utf-8")
        print("----LOGIC EXECUTOR ONLINE----")
        print(res_end)
...
def test_load_le_online(api_v3, db):
    ...
    response = api_v3.init_dialog(
        msisdn="55555", agent="release", api=api_v3, default_logic=False
    )
    ...
    dialog_id = db.select_data(
        table="dialog", column="uuid", sdata="id", data=str(dialog_uuid)
    )[0][0]
...
def test_load_le_online_count(api_v3, db, clear_queue_new):
    ...
    while True:
        result_all = db.db_conn(
            f"SELECT count(id) from call where agent_id ='{agent_id}' and result IN ('pending','queued', null) "
        )
        result = db.db_conn(
            f"SELECT count(id) from call where agent_id ='{agent_id}' and result = 'pending' "
        )
        print(
            time.time(),
            "Всего звонков -",
...
def clear_queue_new(request, api_v3, pool_api_v3):
    def clear():
        params_agent_uuid = {
            "agent_uuid": api_v3.test_data["data_release_run"]["agent_uuid"]
        }
        api_v3.request_send(
            method="PUT",
            path=api_v3.path_end_point["put_change_agent_settings"],
