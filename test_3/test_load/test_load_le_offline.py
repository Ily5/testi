...
class TestLeOfflineLoad:
    def test_load_logic_offline(
        self, api_v3, pool_api_v3, creation_queue_dialog, params_agent_uuid
    ):
        path = pool_api_v3.path_end_point["get_all_dialog_queue"]
        params = {**{"page": "1", "by_count": "10000"}, **params_agent_uuid}
        ...
        res = api_v3.request_send(
            ...
            json={"total_channel_limit": 100000},
...
def check_log(when: str, user: str, **kwargs):
    ...
    if when.lower() in "before":
        ...
        try:
            while True:
                result = client.exec_command(
                    f'tail -n 2  /var/log/ivr/{kwargs["log_name"]}'
                )
                data = result[1].read() + result[2].read()
                result_new = client.exec_command(
                    f'tail -n 2  /var/log/ivr/{kwargs["log_name"]}'
                )
