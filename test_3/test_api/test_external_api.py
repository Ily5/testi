...
class TestExternalApi:
    ...
    def test_get_token_no_valid(self, api_v3, login):
        ...
        response = requests.request(
            method="POST", url=request_url, auth=(login, str(password))
        )
    ...
    def test_change_agent_settings_valid(
        ...
        response = api_v3.request_send(
            method="PUT", path=path, params=params_agent_uuid, json=data
        )
        assert response.status_code == 200
    ...
    def test_edit_rule_in_agent_valid(self, api_v3, create_new_rule_agent, day):
        path = api_v3.path_end_point["agent_call_rules"]
        data = {
            "time_slot_uuid": create_new_rule_agent,
            "day": day,
            "not_before": "10:01:01",
            "not_after": "21:59:59",
        }
    ...
    def test_edit_initial_entity_agent_valid(
        ...
        path = api_v3.path_end_point["agent_initial_entities"]
        data = {
            "initial_entity_uuid": create_new_initial_entity_agent,
    ...
    def test_add_dialogs_bulk_uuid(
        ...
        response = api_v3.request_send(
            method="POST", path=path, params=params_agent_uuid, json=data_dialogs
        )
        time.sleep(2)

        bulk_uuid = response.json()["bulk_uuid"]
        response_new = api_v3.request_send(
            method="POST",
            path=path,
            params={**params_agent_uuid, **{"bulk_uuid": bulk_uuid}},
            json=data_dialogs,
        )
        try:
    ...
    @allure.title(
        "Добавление диалга к уже существующему набору диалогов по несуществущему bulk_uuid"
    )
    ...
    @allure.title(
        "Добавление диалга к уже существующему набору диалогов с невалидным bulk_uuid"
    )
    ...
    @allure.title(
        "Получение статуса множественной загрузки параметров диалога, валидные данные"
    )
    ...
    @allure.title(
        "Получение результата множественной загрузки параметров диалога, валидные данные"
    )
    ...
    @allure.title(
        "Получение output entities статистики по заданомму временному интервалу, валидные данные"
    )
    def test_get_output_entities_time_slot_valid(self, api_v3):
        ...
        data = {
            ...
            "start": "2020-11-26 10:08:05.0",
            "end": "2020-12-10 12:08:05.0",
        }
    ...
    def test_remove_one_dialogs_queue(
        ...
        response = api_v3.request_send(
            method="POST",
            path=api_v3.path_end_point["remove_queue_dialog_2"],
            json={},
    ...
    def test_remove_all_dialogs_queue(
        ...
        response_after = api_v3.request_send(method="POST", path=path, json=data)
        assert response.status_code == 200
        assert int(response_before.json()["total_count"]) > int(
            response_after.json()["total_count"]
        )
    ...
    def test_remove_one_calls_queue(
        ...
        response = api_v3.request_send(
            method="POST",
            path=api_v3.path_end_point["remove_queue_call"],
            json={},
            params=params_for_remove,
        )
