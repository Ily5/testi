...
class TestPoolApiCalls:
    ...
    def test_drop_queue_calls(self, pool_api_v3, params_agent_uuid):
    ...
    def test_get_full_queue_calls(self, pool_api_v3, params_agent_uuid):
        path = pool_api_v3.path_end_point['get_full_queue']
        response = pool_api_v3.request_send(path=path, params=params_agent_uuid)
        assert response.status_code == 200

    @allure.feature('Получить все валидные звонки')
    def test_get_all_valid_calls(self, pool_api_v3, params_agent_uuid):
    ...
    def test_defer_one_call_valid(self, pool_api_v3, params_agent_uuid):
    ...
    def test_cancel_one_call_valid(self, pool_api_v3, params_agent_uuid):
        with allure.step('Получаем список звонков в очереди'):
            response_list_queue_calls = pool_api_v3.request_send(
                path=pool_api_v3.path_end_point['get_list_queue_calls'],
                params={**{"page": "1",
    ...
    def test_cancel_all_calls_valid(self, pool_api_v3, params_agent_uuid):
        path = pool_api_v3.path_end_point['get_list_queue_calls']
...
class TestPoolApiDialog:
    ...
    def test_get_dialog_queue_size_valid(self, pool_api_v3, params_agent_uuid):
        path = pool_api_v3.path_end_point['get_dialog_queue_size']
        response = pool_api_v3.request_send(path=path, params=params_agent_uuid)
        assert response.status_code == 200

    @allure.feature('Отложить один диалог, валидный')
    def test_defer_one_dialog_valid(self, pool_api_v3, params_agent_uuid):
    ...
    def test_defer_all_dialogs(self, pool_api_v3, params_agent_uuid):
        path = pool_api_v3.path_end_point['defer_dialog']
    ...
    def test_return_all_dialog_valid(self, pool_api_v3, params_agent_uuid):
    ...
    def test_return_one_dialog_valid(self, pool_api_v3, params_agent_uuid):
        with allure.step('Получение списка всех диалогов'):
            response_list_dialogs = pool_api_v3.request_send(path=pool_api_v3.path_end_point['get_all_dialog_queue'],
                                                             params={**{"page": "1",
                                                                        "by_count": "100"}, **params_agent_uuid})
